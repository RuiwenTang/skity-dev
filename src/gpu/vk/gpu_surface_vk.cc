...
void GPUSurfaceVK::OnFlush() {
  ...
  if (!external_semaphores_.empty()) {
    ...
    submit_info_.wait_semaphores.resize(
        submit_info_.wait_semaphores.size() - count);
    submit_info_.wait_stage_masks.resize(
        submit_info_.wait_stage_masks.size() - count);
